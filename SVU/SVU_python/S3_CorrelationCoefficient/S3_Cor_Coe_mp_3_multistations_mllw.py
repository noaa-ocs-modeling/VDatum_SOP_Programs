...
# -*- coding: utf-8 -*-
"""
Parallel MATLAB-aligned MLLW correlation to multiple stations (per-shard writer; safe)

MLLW event logic (station & nodes):
  • Use ONLY −2 (LLW) lows: select packed lows < -50, decode by +100.
  • Discard other lows.
...
Output (per-shard):
  • NetCDF with coef(node_in_shard, station=K), coords include absolute node ids and station IDs.

Typical SLURM call:
  python S3_Cor_Coe_MLLW_matlab_aligned_mp_multiStation.py \
    --ll TADll.nc --gage gageinfo.csv \
    --outdir out_mllw_matlabAligned_multi \
    --out-nc coef_full_mllw_matlabAligned_multi_SLICE_${SLURM_ARRAY_TASK_ID}.nc \
...
def _decode_lows_llw_only(t_vec, z_vec):
    """
    Packed lows vector: keep ONLY LLW events (−2): z < -50, then decode by +100.
...
def _fit_spline(t_days, z):
    """
    LSQ cubic spline; ~1 break per 2 days over the node's own span.
...
# ---------------- Station prep ----------------
def _prepare_stations_MLLW(v_ll, station_idx_list, rday=1.0, np_per_hour=3, gap_max_days=3.0):
    """
    Build MATLAB-style grid and station curve for each station using MLLW (−2 only).
    """
    ...
    for ks, st_idx in enumerate(station_idx_list):
        ...
        t_days_st, z_st, n_ev_st, max_gap_st, _ = _decode_lows_llw_only(row_st[:240], row_st[240:])
...
def _compute_block_multi(rows_batch, station_grids, station_curves, gap_max_days):
    """
    For each node in the batch: decode MLLW-only, fit once, evaluate on each station grid, compute r.
    """
    ...
    for i in range(B):
        row = rows_batch[i]
        t_days, z_vals, n_ev, max_gap, _ = _decode_lows_llw_only(row[:240], row[240:])
...
def main():
    ...
    # station prep (MLLW)
    print("[prep] building station grids & curves (MLLW, MATLAB-aligned)…")
    stations_ok, station_grids, station_curves, station_breaks = _prepare_stations_MLLW(
    ...
    xr.Dataset(
        ...
        attrs=dict(datum="mllw", ns=i0, ne=i1, note="Per-shard MLLW coefficients; MATLAB-aligned; multi-station")
