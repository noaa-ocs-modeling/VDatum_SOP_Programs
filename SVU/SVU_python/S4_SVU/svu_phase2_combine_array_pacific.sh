...
# Only chunk 0 will run the combine step
if [[ "${SLURM_ARRAY_TASK_ID}" == "0" ]]; then
    echo "=== Combining chunks for all datums ==="
    for VAR in "${DATUMS[@]}"; do
        python3 svu_phase2_combine_pacific.py \
            --runid $RUNID \
            --datum $VAR \
            --nchunks $CHUNKS \
            --path_pre $PRE
...
echo "===Finished combining all 6 datums==="
